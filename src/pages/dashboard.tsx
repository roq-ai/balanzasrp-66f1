...
import { useRoqPlatformUserProfiles, useProductorCount, useCerealCount, useTransportistaCount } from 'lib/roq';

import { TransportistaListPage as ShowCaseList } from 'pages/transportistas';

function DashboardPage() {
  const { session } = useSession();

  const productorStat = useProductorCount();

  const cerealStat = useCerealCount();

  const transportistaStat = useTransportistaCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Productors',
      result: productorStat?.data,
      isLoading: productorStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Cereals',
      result: cerealStat?.data,
      isLoading: cerealStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Transportistas',
      result: transportistaStat?.data,
      isLoading: transportistaStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Transportistas
                </Text>
                <NextLink href={`transportistas`} passHref legacyBehavior>
                  <Button
